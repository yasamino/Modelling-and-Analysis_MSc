...
def Elongation_distribution( Plot_distribution = True , bin_n=50 ):
    for filename in Files:
        f_count=0
        with celldiv.TrajHandle(filename) as th:
            try:
                for frame in range(int(th.maxFrames/nSkip)+2): # i for each frame written to file
                    ...
                    if Plot_distribution:
                        plt.clf()
                        plt.hist(Elongation_of_hard_cells, bins=bin_n, alpha=0.5, label='Hard cells', color='r', weights=np.ones_like((Elongation_of_hard_cells)) / len(Elongation_of_hard_cells) *100 )
                        plt.hist(Elongation_of_soft_cells, bins=bin_n, alpha=0.5, label='Soft cells', color='b', weights=np.ones_like((Elongation_of_soft_cells)) / len(Elongation_of_soft_cells) *100 )
                        # x_max = 0.6
...
def Animate_vector_field_elongation_vector(size_z = 28, frame_rate = 5, differenciate_soft_hard = False):
    ...
    def visualisation_animation(f,th,frame_number , Filename , size , ax , view = 'XY' , z = 1.9):
        ...
        s1 = np.where(m < 0)[0]
        s2 = np.where(m >= 0 )[0]
        # c = [ f[i][:180] for i in range(len(m)) ] #-> Not needed if differenciating by type 
        
        soft = [ f[i][:180] for i in s1 ]
        for cell in soft:
            hull = ConvexHull(cell[:,1:]) # Finding the boundary points
            Area = hull.volume
            p = cell[hull.vertices,1:]
    ...
    for filename in Files:
        with celldiv.TrajHandle(filename) as th:
            ...
            #ani.save('elong_colorma_{}_{}.gif'.format(view, filename[:-4]), writer='Pillow', fps=frame_rate)
            ani.save('elong_colorma_{}_{}.mp4'.format(view, filename[:-4]), writer='ffmpeg', fps=frame_rate, dpi=200)
